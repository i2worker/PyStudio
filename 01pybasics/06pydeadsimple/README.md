<h2 align="center">Python编程：从新手到高手</h2>

<p align="center"><a href="https://book.douban.com/subject/37454087"><img src="img/001.jpg" width="300" alt="Python编程：从新手到高手"></a></p>

- 作者：\[美\] 贾森·C·麦克唐纳
- 译者：周琦
- 评分：9.2

<hr style="height:1px; border:none; background-color:#d1d9e0b3;">

- **第一部分　Python环境**
    - [第01章　Python的哲学](./第01章-Python的哲学.ipynb)
    - [第02章　Python开发环境](./第02章-Python开发环境.ipynb)
    - [第03章　语法速成课程](./第03章-语法速成课程.ipynb)
    - [第04章　项目结构和代码导入](./第04章-项目结构和代码导入.ipynb)
- **第二部分　基本结构**
    - [第05章　变量和数据类型](./第05章-变量和数据类型.ipynb)
    - [第06章　函数和匿名函数](./第06章-函数和匿名函数.ipynb)
    - [第07章　类和对象](./第07章-类和对象.ipynb)
    - [第08章　错误和异常](./第08章-错误和异常.ipynb)
- **第三部分　数据和流程**
    - [第09章　集合与迭代](./第09章-集合与迭代.ipynb)
    - [第10章　生成器和推导式](./第10章-生成器和推导式.ipynb)
    - [第11章　文本输入/输出和上下文管理](./第11章-文本输入输出和上下文管理.ipynb)
    - [第12章　二进制和序列化](./第12章-二进制和序列化.ipynb)
- **第四部分　高级概念**
    - [第13章　继承和混入](./第13章-继承和混入.ipynb)
    - [第14章　元类和抽象基类](./第14章-元类和抽象基类.ipynb)
    - [第15章　自省和泛型](./第15章-自省和泛型.ipynb)
    - [第16章　异步和并发](./第16章-异步和并发.ipynb)
    - [第17章　线程和并行](./第17章-线程和并行.ipynb)
- **第五部分　超越代码**
    - 第18章　打包和分发(略)
    - 第19章　调试和日志(略)
    - 第20章　测试和剖析(略)
    - 第21章　前路迢迢(略)
